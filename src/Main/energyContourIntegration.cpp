...
{
  double timeEnergyContourIntegration_1=get_rtc();
...
// solve single site problem
    double timeSingleScatterers=get_rtc();
...
#pragma omp parallel for default(none) shared(local,lsms,eGroupIdx,ig,egrd,solution,vr_con)
  ...
  //VT_USER_END("singleSiteSolver");
    ...
    timeSingleScatterers=get_rtc()-timeSingleScatterers;
...
//        int nplmax=solution[iie][i].zlr.l_dim2()-1;
  ...
  //VT_USER_END("Densities");
    ...
    timeCalcDensities=get_rtc()-timeCalcDensities;
    if(lsms.global.iprint>=0) printf("timeCalculateDensities = %lf sec\n",timeCalcDensities);
  //}
  }
  }
  timeEnergyContourIntegration_2=get_rtc()-timeEnergyContourIntegration_2;
