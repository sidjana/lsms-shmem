...
// loop over all sites:
  for(int i=0; i<crystal.num_atoms; i++)
  {
    ...
    if(node==comm.comm.rank) // local atom type
    {
...
// add to commTmatFrom
        if(crystal.types[crystal.type[tempLIZ[j].idx]].node!=comm.comm.rank) // need this from remote node
        {
...
// i.e find min distance between atom and local atoms and compare with max liz radius
      ...
      ret=0;
      while(nodeIsInList(comm.comm.rank,tempLIZ,tempNumLIZ,crystal,ret)) // the remote node needs the tmat from our node
      {
...
// count the nodes in toList and fromList
  if(lsms.global.iprint>0) printf("toList.size()=%d\n",toList.size());
  int numToNodes=0;
  if(toList.size()>0)
  {
    numToNodes=1;
  ...
  if(lsms.global.iprint>0) printf("fromList.size()=%d\n",fromList.size());
  int numFromNodes=0;
  if(fromList.size()>0)
  {
    numFromNodes=1;
