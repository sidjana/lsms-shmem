...
int loadPotentials(LSMSCommunication &comm,LSMSSystemParameters &lsms, CrystalParameters &crystal, LocalTypeInfo &local)
{
  ...
  if(lsms.pot_in_type==-1)  return initializeNewPotentials(comm,lsms,crystal,local); // start without potential files
...
// loop over all atom types:
    for(int i=0; i<crystal.num_types; i++)
    {
      ...
      if(lsms.pot_in_type==0) // LSMS_1 style HDF5
      {
        ...
        if(crystal.types[i].node==comm.comm.rank)
        {
          readSingleAtomData_hdf5(fid_1,local.atom[crystal.types[i].local_id]);
        } else {
          readSingleAtomData_hdf5(fid_1,pot_data);
          communicateSingleAtomData(comm, comm.comm.rank, crystal.types[i].node, crystal.types[i].local_id, pot_data);
        }
        H5Gclose(fid_1);
        ...
        if(crystal.types[i].node==comm.comm.rank)
        {
          ...
          printf("master entering send/recv:%d node%d local_id=%d, local.num_local=%d\n",comm.comm.rank,crystal.types[i].node, crystal.types[i].local_id,local.num_local);
          communicateSingleAtomData(comm, comm.comm.rank, crystal.types[i].node, crystal.types[i].local_id, pot_data);
        }
      }
    }
  } else { // comm.comm.rank!=0
    int local_id=0; // unused var for slave
    for(int i=0; i<local.num_local; i++)
    {
      communicateSingleAtomData(comm, 0, comm.comm.rank, local_id, pot_data);
...
// calculate global properties: chempot, zvaltss, ...
  ...
  static Real fspace[2];
  fspace[0]=lsms.zvaltss;
  fspace[1]=lsms.chempot;
  
  globalSum_real(comm,fspace,2);
...
// calculate global properties: chempot, zvaltss, ...
  ...
  static Real fspace_1[2];
  fspace_1[0]=lsms.zvaltss;
  fspace_1[1]=lsms.chempot;
  
  globalSum_real(comm,fspace_1,2);
  
  lsms.zvaltss=fspace_1[0]; // /Real(lsms.num_atoms);
  lsms.chempot=fspace_1[1]/Real(lsms.num_atoms);
}
...
// loop over all atom types:
    for(int i=0; i<crystal.num_types; i++)
    {
      ...
      if(lsms.pot_out_type==0) // LSMS_1 style HDF5
      {
        ...
        if(crystal.types[i].node==comm.comm.rank)
        {
          ...
          communicateSingleAtomData(comm, crystal.types[i].node, comm.comm.rank, crystal.types[i].local_id, pot_data,i);
          writeSingleAtomData_hdf5(fid_1,pot_data);
        }
        H5Gclose(fid_1);
        ...
        if(crystal.types[i].node==comm.comm.rank)
        {
          ...
          int local_id;
          communicateSingleAtomData(comm, crystal.types[i].node, comm.comm.rank, local_id, pot_data,i);
    ...
    for(int i=0; i<local.num_local; i++)
    {
      communicateSingleAtomData(comm, comm.comm.rank, 0, i, local.atom[i],local.global_id[i]);
    }
  }
