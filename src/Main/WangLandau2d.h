...
class WL2dEvecGenerator : public EvecGenerator
{
 public:
  ...
  bool generateEvec(int instance, double *evecs, double energy, double magnetization) {bool h; return generateEvec(instance, evecs, energy, magnetization, &h);}
  bool generateEvec(int instance, double *evecs) {std::cerr<<"Need energy for WL2dEvecGenerator\n"; exit(1);}
...
template<class RNG>
bool WL2dEvecGenerator<RNG>::generateEvec(int instance, double *evecs, double energy, double magnetization,
					  bool *accepted)
{
  bool accept_step;
  ...
  *accepted=false;
  ...
  {
    dos.extendToY(gridX,magnetization-dosKernel.getWidthY()); histo.extendToY(gridX,magnetization-histoKernel.getWidthY());
    dos.extendToY(gridX,magnetization+dosKernel.getWidthY()); histo.extendToY(gridX,magnetization+histoKernel.getWidthY());
