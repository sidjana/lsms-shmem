...
{
  int q=crystal.num_types/comm.comm.size;
  int r=crystal.num_types%comm.comm.size;
  ...
  for(int i=r; i<comm.comm.size; i++)
  {
    for(int j=0; j<q; j++)
    {
      crystal.types[idx].node=nodeNumber;
  ...
  printf("r=%d q=%d myrank=%d crystal.num_types=%d\n",r,q,comm.comm.rank, crystal.num_types);
  int num_local=0;
  for(int i=0; i<crystal.num_types; i++)
    if(crystal.types[i].node==comm.comm.rank) num_local++;
